...
public class HttpCookieOAuth2AuthorizationRequestRepository implements AuthorizationRequestRepository<OAuth2AuthorizationRequest> {
    ...
    private static final String OAUTH2_AUTHORIZATION_REQUEST_COOKIE_NAME = "oauth_auth_request";

    private static final String REDIRECT_PARAM_COOKIE_NAME = "redirect_uri";

    private static final int COOKIE_MAX_AGE_IN_SECONDS = 180;
    ...
    public void saveAuthorizationRequest(OAuth2AuthorizationRequest authorizationRequest, HttpServletRequest request, HttpServletResponse response) {
        ...
        CookieUtils.setCookie(response, OAUTH2_AUTHORIZATION_REQUEST_COOKIE_NAME, CookieUtils.searlizeCookie(authorizationRequest), COOKIE_MAX_AGE_IN_SECONDS);
        String redirectUriAfterLogin = request.getParameter(REDIRECT_PARAM_COOKIE_NAME);
        if (redirectUriAfterLogin != null) {
            CookieUtils.setCookie(response, REDIRECT_PARAM_COOKIE_NAME, redirectUriAfterLogin, COOKIE_MAX_AGE_IN_SECONDS);
        }
    }
